...
export default function DiscussionDetail() {
  ...
  const handleLikeDiscussion = async () => {
    ...
    try {
      ...
      const likedBy: string[] = discussionSnap.data()?.likedBy || []
      const alreadyLiked = likedBy.includes(userData.uid)

      await updateDoc(discussionRef, {
        likesCount: increment(alreadyLiked ? -1 : 1),
        likedBy: alreadyLiked ? arrayRemove(userData.uid) : arrayUnion(userData.uid),
      })
      fetchDiscussionAndReplies()
    } catch (error) {
  ...
  const handleLikeReply = async (replyId: string) => {
    ...
    try {
      ...
      const likedBy: string[] = replySnap.data()?.likedBy || []
      const alreadyLiked = likedBy.includes(userData.uid)
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                ...
                <div className="flex items-center gap-4 pt-4 border-t">
                  <Button variant="ghost" size="sm" onClick={handleLikeDiscussion} className={`gap-2 ${discussion.likedBy?.includes(userData?.uid || "") ? "text-blue-600" : ""}`}>
                    <ThumbsUp className={`w-4 h-4 ${discussion.likedBy?.includes(userData?.uid || "") ? "fill-blue-600" : ""}`} />
                      ...
                      <div key={reply.id} className="flex gap-3 p-4 rounded-lg bg-muted/50">
                        ...
                        <div className="flex-1 space-y-2">
                          ...
                          <Button
                            ...
                            className={`gap-2 h-8 ${reply.likedBy?.includes(userData?.uid || "") ? "text-blue-600" : ""}`}
                          >
                            <ThumbsUp className={`w-3 h-3 ${reply.likedBy?.includes(userData?.uid || "") ? "fill-blue-600" : ""}`} />
