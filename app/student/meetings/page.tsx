...
export default function MeetingsPage() {
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      if (sid) {
        try {
          ...
          if (supervisor) setSupervisorName(supervisor.name || t("notSpecified"))
        } catch (error) {
    ...
    const unsubMeetings = onSnapshot(
      query(collection(db, "meetings"), where("studentId", "==", userData.uid)),
      (snap) => {
        const data = snap.docs.map((d) => ({ id: d.id, ...d.data() }))
        setMeetings(data.sort((a: any, b: any) => (b.date?.seconds || 0) - (a.date?.seconds || 0)))
        setLoading(false)
      },
      (err) => { console.error("Error fetching meetings:", err); setLoading(false) }
    )

    // Real-time: meeting requests
    const unsubRequests = onSnapshot(
      query(collection(db, "meeting_requests"), where("studentId", "==", userData.uid)),
      (snap) => {
        const data = snap.docs.map((d) => ({ id: d.id, ...d.data() }))
        setMeetingRequests(data.sort((a: any, b: any) => (b.createdAt?.seconds || 0) - (a.createdAt?.seconds || 0)))
      }
    )

    return () => { unsubMeetings(); unsubRequests() }
  }, [userData?.uid])
