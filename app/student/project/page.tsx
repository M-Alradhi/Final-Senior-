...
export default function StudentProject() {
  ...
  useEffect(() => {
    ...
    const unsubIdeas = onSnapshot(
      query(collection(db, "projectIdeas"), where("studentId", "==", userData.uid)),
      () => fetchProject()
    )
    // Real-time: project (if projectId known)
    let unsubProject: (() => void) | null = null
    if (userData?.projectId) {
      unsubProject = onSnapshot(doc(db, "projects", userData.projectId), () => fetchProject())
    }
    // Real-time: team projects
    const unsubTeam = onSnapshot(
      query(collection(db, "projects"), where("teamMembers", "array-contains", userData.uid)),
      () => fetchProject()
    )
  ...
  const getStatusText = (status: string) => {
    ...
    if (status === "pending_team_approval") return t("waitingForTeamApproval")
    if (status === "pending_approval") return t("waitingForTeamApproval")
