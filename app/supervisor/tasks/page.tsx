...
export default function SupervisorTasks() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      // جلب المشاريع الأساسية (المشرف الرئيسي) والثانوية (coSupervisorId أو supervisors array)
      ...
      const allProjectDocs = [...projectsSnapshot.docs, ...projectsSnapshotSecondary.docs].filter(
        (d, i, arr) => arr.findIndex((x) => x.id === d.id) === i,
      )

      allProjectDocs.forEach((d) => {
        const projectData = d.data()
        newProjectsMap.set(d.id, {
          id: d.id,
      ...
      // بناء قائمة الطلاب — كل طالب يُضاف مرة واحدة لكل مشروع هو منتسب له
      const studentsData: Student[] = []
      const processedPairs = new Set<string>() // مفتاح مركّب: studentId+projectId

      newProjectsMap.forEach((project) => {
        const projectStudentIds = new Set([project.studentId, ...(project.studentIds || [])].filter(Boolean))

        projectStudentIds.forEach((studentId: string) => {
          const pairKey = `${studentId}::${project.id}`
    ...
    // المهام التي أنشأها هذا المشرف (رئيسي أو ثانوي) — where supervisorId == uid
    ...
    // المهام المرتبطة بمشاريع هذا المشرف الثانوي (حتى لو أنشأها المشرف الرئيسي)
    getDocs(query(collection(db2, "projects"), where("coSupervisorId", "==", userData.uid)))
      .then((projSnap) => {
            ...
            <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
              ...
              <form onSubmit={handleCreateTask} className="space-y-4">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setTaskForm({
                          ...
                          studentId: "",
                          projectId: e.target.checked ? taskForm.projectId : "",
                        })
                      }
                      className="rounded"
                    />
                ...
                {taskForm.assignToAllMembers ? (
                  <div className="space-y-2">
                    ...
                    >
                      ...
                      <SelectContent>
                        {Array.from(new Set(students.filter((s) => s.projectId).map((s) => s.projectId))).map((projectId) => {
                    ...
                    {taskForm.projectId && (
                      <p className="text-sm text-muted-foreground">
                        {t("taskAddTo")}
                        {students.filter((s) => s.projectId === taskForm.projectId).length} {t("student")}
                      </p>
                    )}
                  </div>
                ) : (
                ...
                <div className="space-y-3 pt-2">
                  ...
                  <p className="text-xs text-muted-foreground">{t("taskFilesDescription")}</p>
        ...
        {loading ? (
          ...
          <Card className="border-dashed">
            <CardContent className="flex flex-col items-center justify-center py-16">
              ...
              <p className="text-sm text-muted-foreground">
                {t("ThereAreNoStudentsCurrentlyRegisteredUnderYourSupervision")}
              </p>
            </CardContent>
          </Card>
        ) : (
          <Tabs defaultValue="by-student" className="space-y-4">
            <TabsList className="grid w-full grid-cols-3 text-xs sm:text-sm">
              <TabsTrigger value="by-student" className="px-1 sm:px-3">
                {t("byStudent")}
              </TabsTrigger>
              <TabsTrigger value="by-project" className="px-1 sm:px-3">
                {t("byProject")}
              </TabsTrigger>
              <TabsTrigger value="by-status" className="px-1 sm:px-3">
                {t("byStatus")}
              </TabsTrigger>
            </TabsList>
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
                        <div className="flex items-center gap-3 flex-1">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <CardDescription className="truncate">
                              {student.projectTitle || t("withoutProject")}
                            </CardDescription>
                          </div>
                        </div>
                        <div className="flex sm:flex-col items-center sm:items-end gap-3 sm:gap-1 sm:text-left">
                          ...
                          <p className="text-xs text-muted-foreground">
                            {studentTasks.filter((t) => t.status === "graded").length} {t("from")} {studentTasks.length}{" "}
                            {t("task")}
                          </p>
                            ...
                            >
                              <div className="flex-1 space-y-1.5">
                                ...
                                <div className="flex flex-wrap items-center gap-2 sm:gap-4 text-xs text-muted-foreground">
                                  <span>
                                    {t("maxGrade")}: {task.maxGrade}
                                  </span>
                                  <span>
                                    {t("weight")}: {task.weight}%
                                  </span>
                                  <span>
                                    {t("sendd")}: {formatDate(task.dueDate)}
                                  </span>
                                </div>
                                {task.description && (
                                ...
                                {task.status === "graded" ? (
                                  <div className="flex sm:flex-col items-center sm:items-end gap-2 sm:gap-0">
                                    <div className="text-xl sm:text-2xl font-bold text-green-600">
                                      {task.grade}
                                      <span className="text-xs text-muted-foreground font-normal">/{task.maxGrade}</span>
                                    </div>
                                    <Button
                                      size="sm"
                                      variant="ghost"
                  ...
                  >
                    <CardHeader>
                      ...
                      <div className="flex gap-3 mt-3">
                        <Badge
                          variant="secondary"
                          className="bg-amber-100 text-amber-700 dark:bg-amber-950/30 dark:text-amber-400 gap-1"
                        >
                          <Clock className="w-3 h-3" />
                        ...
                        <Badge
                          variant="secondary"
                          className="bg-blue-100 text-blue-700 dark:bg-blue-950/30 dark:text-blue-400 gap-1"
                        >
                          <Upload className="w-3 h-3" />
                        ...
                        <Badge
                          variant="secondary"
                          className="bg-green-100 text-green-700 dark:bg-green-950/30 dark:text-green-400 gap-1"
                        >
                          <CheckCircle2 className="w-3 h-3" />
                              ...
                              >
                                <div className="flex items-center justify-between mb-2">
                                  ...
                                  <div className="flex items-center gap-2 text-xs text-muted-foreground">
                                    <span>
                                      {t("grade")}: {firstTask.maxGrade}
                                    </span>
                                    <span>•</span>
                                    <span>
                                      {t("weight")}: {firstTask.weight}%
                                    </span>
                                    <span>•</span>
                                    <span>
                                      {t("sendd")}: {formatDate(firstTask.dueDate)}
                                    </span>
                                  </div>
                                </div>
                      ...
                      {submittedTasks.map((task) => (
                        ...
                        >
                          <div className="flex-1">
                            ...
                            <p className="text-sm text-muted-foreground">
                              {t("taskSubmitted")}: {formatDate(task.submittedAt)} • {t("maxGrade")}: {task.maxGrade} •{" "}
                              {t("weight")}: {task.weight}%
                            </p>
                          </div>
                      ...
                      {pendingTasks.map((task) => (
                        <div key={task.id} className="flex items-center justify-between p-4 rounded-lg border">
                          <div className="flex-1">
                            ...
                            <p className="text-sm text-muted-foreground">
                              {t("taskSubmitted")}: {formatDate(task.dueDate)} • {t("maxGrade")}: {task.maxGrade} •{" "}
                              {t("weight")}: {task.weight}%
                            </p>
                          </div>
                        </div>
                ...
                <Card className="border-2">
                  <CardHeader>
                    ...
                    <CardDescription className="flex items-center gap-3 flex-wrap">
                      <span>
                        {t("student")}: {selectedTask.studentName}
                      </span>
                      <span>•</span>
                      <span>
                ...
                {selectedTask.status !== "pending" && (
                  <Card className="border-2 bg-blue-50/50 dark:bg-blue-950/10">
                    <CardHeader>
                      ...
                      <CardDescription>
                        {t("submittedTasks")}: {formatDate(selectedTask.submittedAt)}
                      </CardDescription>
                    </CardHeader>
