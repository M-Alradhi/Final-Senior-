...
import { collection, getDocs, onSnapshot, query, where, setDoc, doc } from "firebase/firestore"
...
export default function CoordinatorSupervisors() {
  ...
  const fetchSupervisors = async () => {
    try {
      ...
      const supervisorsData = await Promise.all(
        supervisorsSnapshot.docs.map(async (docSnap) => {
          const supervisorData = { id: docSnap.id, ...docSnap.data() }

          // Count as primary supervisor
          const primaryQuery = query(collection(db, "projects"), where("supervisorId", "==", docSnap.id))
          const primarySnap = await getDocs(primaryQuery)

          // Count as co-supervisor
          const coQuery = query(collection(db, "projects"), where("coSupervisorId", "==", docSnap.id))
          const coSnap = await getDocs(coQuery)

          // Count via supervisorIds array
          const supIdsQuery = query(collection(db, "projects"), where("supervisorIds", "array-contains", docSnap.id))
          const supIdsSnap = await getDocs(supIdsQuery)

          // Merge unique project IDs من كل المصادر
          const projectIds = new Set<string>()
          primarySnap.docs.forEach((d) => projectIds.add(d.id))
          coSnap.docs.forEach((d) => projectIds.add(d.id))
          supIdsSnap.docs.forEach((d) => projectIds.add(d.id))

          // Count students under this supervisor
          const studentsQuery = query(collection(db, "users"), where("role", "==", "student"), where("supervisorId", "==", docSnap.id))
  ...
  useEffect(() => {
    ...
    // Real-time: supervisors via users collection
    const supervisorsQuery = query(collection(db, "users"), where("role", "==", "supervisor"))
    const unsubSup = onSnapshot(supervisorsQuery, async () => {
      await fetchSupervisors()
    })

    // Real-time: students
    const studentsQuery = query(collection(db, "users"), where("role", "==", "student"))
    const unsubStu = onSnapshot(studentsQuery, async (snap) => {
      const studentsData = snap.docs.map((d) => ({ id: d.id, ...d.data() }))
      setStudents(studentsData)
      setLoading(false)
    })

    // Real-time: projects (to update project counts on supervisors)
    const projectsQuery = collection(db, "projects")
    const unsubProj = onSnapshot(projectsQuery, async () => {
      await fetchSupervisors()
    })

    return () => { unsubSup(); unsubStu(); unsubProj() }
  }, [])
