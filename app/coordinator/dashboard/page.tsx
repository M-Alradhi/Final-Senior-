...
export default function CoordinatorDashboard() {
  ...
  useEffect(() => {
      ...
      try {
        ...
        projectsData.forEach((project: any) => {
          if (project.supervisorId) {
            supervisorProjectCounts.set(project.supervisorId, (supervisorProjectCounts.get(project.supervisorId) || 0) + 1)
          }
        })
        const supervisorsWithCounts = supervisorsData
          .map((sup: any) => ({ ...sup, projectCount: supervisorProjectCounts.get(sup.uid) || 0 }))
    ...
    const unsubSupervisors = onSnapshot(
      ...
      (snap) => { supervisorsData = snap.docs.map((d) => ({ id: d.id, ...d.data() })); recalc() }
    )
    const unsubStudents = onSnapshot(
      query(collection(db, "users"), where("role", "==", "student")),
      (snap) => { studentsData = snap.docs.map((d) => ({ id: d.id, ...d.data() })); recalc() }
    )

    return () => { unsubProjects(); unsubSupervisors(); unsubStudents() }
  }, [authLoading])
