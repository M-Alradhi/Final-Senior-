...
export default function CoordinatorProjects() {
  ...
  const handleRemoveStudentFromProject = async (studentId: string) => {
    ...
    try {
      // 1) Remove student's project/supervisor link
      await updateDoc(doc(db, "users", studentId), {
        projectId: null,
        supervisorId: null,
      })

      // 2) Remove from teamMembers in project
      const currentMembers = selectedProject.teamMembers || []
      const updatedMembers = currentMembers.filter((id: string) => id !== studentId)
      await updateDoc(doc(db, "projects", selectedProject.id), {
        teamMembers: updatedMembers,
        isTeamProject: updatedMembers.length > 1,
        studentId: selectedProject.studentId === studentId ? null : selectedProject.studentId,
        studentName: selectedProject.studentId === studentId ? "" : selectedProject.studentName,
      })

      // 3) Delete all tasks assigned to this student in this project
      ...
      await Promise.all(tasksSnap.docs.map((taskDoc) => deleteDoc(doc(db, "tasks", taskDoc.id))))

      // 4) Delete all meetings for this student in this project
      ...
      await Promise.all(meetingsSnap.docs.map((mDoc) => deleteDoc(doc(db, "meetings", mDoc.id))))

      // 5) Delete meeting requests
      const meetReqQuery = query(
        collection(db, "meeting_requests"),
        where("studentId", "==", studentId),
        where("projectId", "==", selectedProject.id)
      )
      const meetReqSnap = await getDocs(meetReqQuery)
      await Promise.all(meetReqSnap.docs.map((mDoc) => deleteDoc(doc(db, "meeting_requests", mDoc.id))))
